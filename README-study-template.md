...
Once you have an API login, edit the file [`.env`](.env) and enter your
...
you can also enter your password into the [`.env`](.env) file as
...
See [`requirements.txt`](requirements.txt) for the package requirements.  To
install them, run:
...
stored in the [`study-config.json`](study-config.json) file.  This file is used
to automatically create a [`Makefile.study`](Makefile.study) with make targets
for each output file.

All Boa queries must reside under the [`boa/`](boa) folder.  There are two
sub-folders under the query folder: [`boa/queries/`](boa/queries) and
[`boa/snippets/`](boa/snippets).  [`boa/queries/`](boa/queries) is where you
...
[`study-config.json`](study-config.json) configuration file.  This file defines
...
[`data/txt/`](data/txt).  You omit that prefix in the file paths listed here.
You can include sub-folders if you want.

The values must have, at a minimum, `query` and `dataset` keys listing the path
to the Boa query stored under [`boa/`](boa) (again, you omit that prefix here,
...
[`data/csv/`](data/csv), with the prefix omitted here) or an object listing the
...
Finally, a query can also indicate if the [`gendupes.py`](bin/gendupes.py)
...
(here, you must provide the [`data/txt/`](data/txt) prefix).  It can also
...
[`tables/kotlin/rq1.tex`](tables/kotlin/rq1.tex).

The steps to add a new analysis are as follows:

1. Create a new Boa query (e.g., `foo.boa`):\
   a. Store the file in [`boa/queries/`](boa/queries) - note you can create
      sub-folders here if you want.\
   b. Add any missing common snippets into [`boa/snippets/`](boa/snippets).\
   c. Add the query into the [`study-config.json`](study-config.json).\
   d. Repeat Step 1 as many times as necessary.
2. Create a new Python script to analyze the data (e.g., `foo.py`) in the top
   folder.
3. Add a new entry to the [`study-config.json`](study-config.json) in the
   `analyses` object.\
   a. The name of the entry is the script filename (e.g., `foo.py`, without
      the [`analyses/`](analyses) prefix).\
...
[`replication-pkg.zip`](replication-pkg.zip), [`data.zip`](data.zip), and
[`data-cache.zip`](data-cache.zip).  The first is the main replication package
and should include all Boa queries, analysis scripts, support scripts, and
additional things like the documentation and Makefile.  The second is the
contents of the [`data/txt/`](data/txt) folder, which is the raw outputs from
Boa.  The third is the processed Parquet file caches.
...
To do so, you will need to create a [`.env`](.env) file with the following
contents:
...
share the [`.env`](.env) file with anyone once you create it!  It will not be
...
[`.zenodo.json`](.zenodo.json) file.  This file can be shared and by default is
